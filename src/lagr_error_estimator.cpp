...
namespace pyoomph
{
  ...
  //====================================================================
  Integral *LagrZ2ErrorEstimator::integral_rec(const unsigned &dim,
  ...
  /// pointer.
  //==========================================================================
  double LagrZ2ErrorEstimator::get_combined_error_estimate(
...
// If there are no errors then we have a problem
  ...
  //======================================================================
  void LagrZ2ErrorEstimator::setup_patches(oomph::Mesh *&mesh_pt,
                                           std::map<oomph::Node *, oomph::Vector<oomph::ElementWithZ2ErrorEstimator *> *> &adjacent_elements_pt,
                                           oomph::Vector<oomph::Node *> &vertex_node_pt)
  {
    ...
    std::map<oomph::Node *, Vector<ElementWithZ2ErrorEstimator *> *>
    ...
    for (unsigned e = 0; e < nelem; e++)
    {
      ...
      for (unsigned n = 0; n < nnod; n++)
      {
        ...
        if (aux_adjacent_elements_pt[nod_pt] == 0)
        {
          ...
          aux_adjacent_elements_pt[nod_pt] = new Vector<ElementWithZ2ErrorEstimator *>;
        }
    ...
    for (unsigned e = 0; e < nelem; e++)
    {
      ...
      {
        oomph::Node *nod_pt = el_pt->vertex_node_pt(n);
        ...
        if (adjacent_elements_pt[nod_pt] == 0)
        {
          ...
          // Create Vector of pointers to its adjacent elements
          adjacent_elements_pt[nod_pt] = new Vector<ElementWithZ2ErrorEstimator *>;

          // Copy across:
          unsigned nel = (*aux_adjacent_elements_pt[nod_pt]).size();
          for (unsigned e = 0; e < nel; e++)
          {
            (*adjacent_elements_pt[nod_pt]).push_back((*aux_adjacent_elements_pt[nod_pt])[e]);
          }
        }
      }
    ...
    // Cleanup
    typedef std::map<oomph::Node *, Vector<ElementWithZ2ErrorEstimator *> *>::iterator ITT;
  ...
  /// a pointer to it.
  //======================================================================
  void LagrZ2ErrorEstimator::get_recovered_flux_in_patch(
  ...
  //==================================================================
  unsigned LagrZ2ErrorEstimator::nrecovery_terms(const unsigned &dim)
  {
  ...
  /// - flux_rec*.dat
  //======================================================================
  void LagrZ2ErrorEstimator::get_element_errors(oomph::Mesh *&mesh_pt,
                                                ...
                                                oomph::DocInfo &doc_info)
  {
    ...
    //====================================================
    std::map<oomph::Node *, Vector<ElementWithZ2ErrorEstimator *> *> adjacent_elements_pt;
    Vector<oomph::Node *> vertex_node_pt;
    ...
    for (int i = itbegin; i < itend; i++)
    {
      ...
      oomph::Node *nod_pt = vertex_node_pt[i];
      ...
      if (nelem >= 2)
      {
        ...
        vector_of_recovered_flux_coefficient_pt_to_send.push_back(recovered_flux_coefficient_pt);
    ...
    MapMatrixMixed<oomph::Node *, int, double> rec_flux_map;

    // Loop over all nodes
    unsigned n_node = mesh_pt->nnode();
    for (unsigned n = 0; n < n_node; n++)
    {
      oomph::Node *nod_pt = mesh_pt->node_pt(n);
    ...
    for (unsigned e = 0; e < nelem; e++)
    {
      ...
      if (!el_pt->is_halo())
      {
        ...
        for (unsigned ipt = 0; ipt < n_intpt; ipt++)
        {
          ...
          if (use_Lagrangian)
          {
            J = dynamic_cast<BulkElementBase *>(el_pt)->J_Lagrangian(s);
            dynamic_cast<SolidFiniteElement *>(el_pt)->interpolated_xi(s, x);
          }
          else
          {
            J = el_pt->J_eulerian(s);
            el_pt->interpolated_x(s, x);
          }
      ...
      for (int iproc = 0; iproc < n_proc; iproc++)
      {
        if (iproc != my_rank) // Not current process, so send
        {
          ...
          if (nelem_haloed != 0)
          {
            ...
            {
              // Find element number
              int element_num = elem_num[dynamic_cast<ElementWithZ2ErrorEstimator *>(haloed_elem_pt[e])];
        ...
        else // iproc=my_rank, so receive errors from others
        {
          for (int send_rank = 0; send_rank < n_proc; send_rank++)
          {
            ...
            {
              Vector<GeneralisedElement *> halo_elem_pt = mesh_pt->halo_element_pt(send_rank);
              ...
              if (nelem_halo != 0)
              {
                ...
                for (int e = 0; e < nelem_halo; e++)
                {
                  // Find element number
                  int element_num =
                      elem_num[dynamic_cast<ElementWithZ2ErrorEstimator *>(halo_elem_pt[e])];
  ...
  //==================================================================
  void LagrZ2ErrorEstimator::doc_flux(oomph::Mesh *mesh_pt,
                                      const unsigned &num_flux_terms,
                                      MapMatrixMixed<oomph::Node *, int, double> &rec_flux_map,
