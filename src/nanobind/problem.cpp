...
*/
	...
	nb::class_<pyoomph::Problem, pyoomph::PyProblemTrampoline>(
		...
		.def_prop_rw("_use_frozen_sparsity_for_bifurcation_tracking", &pyoomph::Problem::get_use_frozen_sparsity_for_bifurcation_tracking, &pyoomph::Problem::set_use_frozen_sparsity_for_bifurcation_tracking,
			 ...
			 "the Hessian is finite-differenced rather than generated, since nothing then bounds what it writes. Left off by default because the augmented pattern is the "
			 "one the handler describes by hand rather than one derived from generated code; turn it on when the assembly time of a tracking run actually matters.")
