...
void PyDecl_Problem(py::module &m)
{
	...
	py_decl_GlobalParam = new py::class_<GiNaC::GiNaCGlobalParameterWrapper>(
		m, "GiNaC_GlobalParam",
...
void PyReg_Problem(py::module &m)
{
	...
	m.def("_write_to_log_file", &pyoomph::write_to_log_file, py::arg("message"),
	...
	py::class_<pyoomph::DofAugmentations>(
		m, "DofAugmentations",
		...
		.def("add_vector", &pyoomph::DofAugmentations::add_vector, py::arg("values"),
		...
		.def("add_scalar", &pyoomph::DofAugmentations::add_scalar, py::arg("value"),
			 "Append a single scalar augmented degree of freedom, initialized with ``value``. Returns its index in the augmented dof vector.")
		.def("add_parameter", &pyoomph::DofAugmentations::add_parameter, py::arg("parameter_name"),
		...
		.def(
			"split", [](pyoomph::DofAugmentations &augs, unsigned startindex, int endindex)
			{
				...
				for (unsigned int i = 0; i < res.size(); i++)
					resA[i] = py::cast(res[i]);
				return resA;
			},
			py::arg("startindex") = 1, py::arg("endindex") = -1,
	...
	py_decl_GlobalParam
		...
		.def(-py::self, "Symbolic negation, returns a GiNaC expression.")

		.def(py::self + py::self, "Symbolic addition of two global parameters, returns a GiNaC expression.")
		.def(py::self + GiNaC::ex(), "Symbolic addition with a GiNaC expression, returns a GiNaC expression.")
		.def(int() + py::self, "Symbolic addition with an int, returns a GiNaC expression.")
		.def(py::self + int(), "Symbolic addition with an int, returns a GiNaC expression.")
		.def(float() + py::self, "Symbolic addition with a float, returns a GiNaC expression.")
		.def(py::self + float(), "Symbolic addition with a float, returns a GiNaC expression.")

		.def(py::self - py::self, "Symbolic subtraction of two global parameters, returns a GiNaC expression.")
		.def(py::self - GiNaC::ex(), "Symbolic subtraction with a GiNaC expression, returns a GiNaC expression.")
		.def(int() - py::self, "Symbolic subtraction with an int, returns a GiNaC expression.")
		.def(py::self - int(), "Symbolic subtraction with an int, returns a GiNaC expression.")
		.def(float() - py::self, "Symbolic subtraction with a float, returns a GiNaC expression.")
		.def(py::self - float(), "Symbolic subtraction with a float, returns a GiNaC expression.")

		.def(py::self * py::self, "Symbolic multiplication of two global parameters, returns a GiNaC expression.")
		.def(py::self * GiNaC::ex(), "Symbolic multiplication with a GiNaC expression, returns a GiNaC expression.")
		.def(int() * py::self, "Symbolic multiplication with an int, returns a GiNaC expression.")
		.def(py::self * int(), "Symbolic multiplication with an int, returns a GiNaC expression.")
		.def(float() * py::self, "Symbolic multiplication with a float, returns a GiNaC expression.")
		.def(py::self * float(), "Symbolic multiplication with a float, returns a GiNaC expression.")

		.def(py::self / py::self, "Symbolic division of two global parameters, returns a GiNaC expression.")
		.def(py::self / GiNaC::ex(), "Symbolic division with a GiNaC expression, returns a GiNaC expression.")
		.def(int() / py::self, "Symbolic division with an int, returns a GiNaC expression.")
		.def(py::self / int(), "Symbolic division with an int, returns a GiNaC expression.")
		.def(float() / py::self, "Symbolic division with a float, returns a GiNaC expression.")
		.def(py::self / float(), "Symbolic division with a float, returns a GiNaC expression.")
		...
		.def(
			...
			py::is_operator(), "Symbolic exponentiation with a GiNaC expression exponent, returns a GiNaC expression.")
		.def(
			"__pow__", [](const GiNaC::GiNaCGlobalParameterWrapper &lh, const int &rh)
			{ return GiNaC::pow(lh, rh); },
			py::is_operator(), "Symbolic exponentiation with an int exponent, returns a GiNaC expression.")
		.def(
			"__pow__", [](const GiNaC::GiNaCGlobalParameterWrapper &lh, const double &rh)
			{ return GiNaC::pow(lh, rh); },
			py::is_operator(), "Symbolic exponentiation with a float exponent, returns a GiNaC expression.")

		.def(
			"__rpow__", [](const GiNaC::GiNaCGlobalParameterWrapper &rh, const int &lh)
			{ return GiNaC::pow(lh, rh); },
			py::is_operator(), "Symbolic exponentiation with an int base and this parameter as exponent, returns a GiNaC expression.")
		.def(
			"__rpow__", [](const GiNaC::GiNaCGlobalParameterWrapper &rh, const double &lh)
			{ return GiNaC::pow(lh, rh); },
			py::is_operator(), "Symbolic exponentiation with a float base and this parameter as exponent, returns a GiNaC expression.")
		.def(
	...
	py::class_<pyoomph::SparseRank3Tensor>(
		m, "SparseRank3Tensor",
		...
		.def(
			"finalize_for_vector_product", [](pyoomph::SparseRank3Tensor &self) -> std::tuple<py::array_t<int>, py::array_t<int>>
			{
		...
		.def(
			"right_vector_mult", [](pyoomph::SparseRank3Tensor &self, const py::array_t<double> &vec) -> py::array_t<double>
			{
				py::buffer_info binfo = vec.request();
				auto ptr = static_cast<double *>(binfo.ptr);
				size_t size = 1;
				for (auto s : binfo.shape)
					size *= s;
	...
	py::class_<pyoomph::CustomResJacInformation>(
		m, "CustomResJacInfo",
		...
		.def(
			"set_custom_residuals", [](pyoomph::CustomResJacInformation &self, py::array_t<double> r)
			{
			...
			py::arg("residuals"), "Set the custom residual vector (length must match Problem.ndof()).")
		.def(
			"set_custom_jacobian", [](pyoomph::CustomResJacInformation &self, py::array_t<double> jvals, py::array_t<int> colindex, py::array_t<int> rowstart)
			{
			...
			py::arg("values"), py::arg("column_index"), py::arg("row_start"),
	...
	py::class_<oomph::AssemblyHandler>(
		m, "AssemblyHandler",
	...
	py::class_<pyoomph::MyFoldHandler, oomph::AssemblyHandler>(
		m, "FoldHandler",
		...
		.def(
			"get_eigenfunction", [](pyoomph::MyFoldHandler *self)
			{
		...
		.def("set_eigenweight", &pyoomph::MyFoldHandler::set_eigenweight, py::arg("weight"),
	...
	py::class_<pyoomph::MyPitchForkHandler, oomph::AssemblyHandler>(
		m, "PitchForkHandler",
		...
		.def("get_base_ndof", &pyoomph::MyPitchForkHandler::get_problem_ndof,
			 "Number of degrees of freedom of the underlying (non-augmented) problem.")
		.def("set_eigenweight", &pyoomph::MyPitchForkHandler::set_eigenweight, py::arg("weight"),
			 "Set the weight factor used to scale the eigenvector normalization equation.");

	py::class_<pyoomph::MyHopfHandler, oomph::AssemblyHandler>(
		m, "HopfHandler",
		...
		.def("set_eigenweight", &pyoomph::MyHopfHandler::set_eigenweight, py::arg("weight"),
	...
	py::class_<pyoomph::AzimuthalSymmetryBreakingHandler, oomph::AssemblyHandler>(
		...
		.def("set_eigenweight", &pyoomph::AzimuthalSymmetryBreakingHandler::set_eigenweight, py::arg("weight"),
			 "Set the weight factor used to scale the eigenvector normalization equations.")
		...
		.def("set_global_equations_forced_zero", &pyoomph::AzimuthalSymmetryBreakingHandler::set_global_equations_forced_zero,
			 py::arg("base_equations"), py::arg("eigen_equations"),
	...
	py::class_<pyoomph::PeriodicOrbitHandler, oomph::AssemblyHandler>(
		...
		.def("set_dofs_to_interpolated_values", &pyoomph::PeriodicOrbitHandler::set_dofs_to_interpolated_values, py::arg("s"),
...
*/
	...
	py::class_<pyoomph::DynamicBulkElementInstance>(
		...
		.def("link_external_data", &pyoomph::DynamicBulkElementInstance::link_external_data,
			 py::arg("name"), py::arg("data"), py::arg("index"), py::arg("full_source_name"),
		...
		.def("get_nodal_field_index", &pyoomph::DynamicBulkElementInstance::get_nodal_field_index, py::arg("name"),
			 "Return the local index of the nodal field ``name`` in this element code, or a negative value if it is not present.")
		.def("get_discontinuous_field_index", &pyoomph::DynamicBulkElementInstance::get_discontinuous_field_index, py::arg("name"),
		...
		.def("has_parameter_contribution", &pyoomph::DynamicBulkElementInstance::has_parameter_contribution, py::arg("parameter_name"),
		...
		.def(
			...
			py::arg("analytic"), py::arg("analytic_positions"),
	...
	py::class_<pyoomph::Problem, pyoomph::PyProblemTrampoline>(
		m, "Problem",
		...
		.def(py::init<>(), "Create a new, empty Problem.")
		...
		.def("assign_eqn_numbers", &pyoomph::Problem::assign_eqn_numbers, py::arg("assign_local_eqn_numbers") = true,
		...
		.def("initialise_dt", (void(pyoomph::Problem::*)(const double &)) & pyoomph::Problem::initialise_dt, py::arg("dt"),
			 "Set all time steps of all time steppers in the problem to ``dt`` and (re-)assign their weights.")
		.def("assign_initial_values_impulsive", (void(pyoomph::Problem::*)(const double &)) & pyoomph::Problem::assign_initial_values_impulsive, py::arg("dt"),
		...
		.def(
			...
			py::arg("active"), "Enable or disable the globally convergent (line-search damped) Newton method.")
		.def_property(
		...
		.def("adaptive_unsteady_newton_solve", (double(pyoomph::Problem::*)(const double &, const double &)) & pyoomph::Problem::adaptive_unsteady_newton_solve,
			 py::arg("dt_desired"), py::arg("epsilon"),
		...
		.def("adaptive_unsteady_newton_solve", (double(pyoomph::Problem::*)(const double &, const double &, const bool &)) & pyoomph::Problem::adaptive_unsteady_newton_solve,
			 py::arg("dt_desired"), py::arg("epsilon"), py::arg("shift_values"),
		...
		.def("unsteady_newton_solve", (void(pyoomph::Problem::*)(const double &, const unsigned &, const bool &, const bool &)) & pyoomph::Problem::unsteady_newton_solve,
			 py::arg("dt"), py::arg("max_adapt"), py::arg("first"), py::arg("shift") = true,
		...
		.def("doubly_adaptive_unsteady_newton_solve",
			 ...
			 py::arg("dt"), py::arg("epsilon"), py::arg("max_adapt"), py::arg("suppress_resolve_after_spatial_adapt_flag"), py::arg("first"), py::arg("shift") = true,
		...
		.def("_arc_length_step", &pyoomph::Problem::arc_length_step, py::arg("parameter_name"), py::arg("ds"), py::arg("max_adapt"),
		...
		.def("_set_arc_length_parameter_derivative", &pyoomph::Problem::set_arc_length_parameter_derivative, py::arg("dp_ds"),
			 "Set the derivative of the continuation parameter with respect to arclength.")
		.def("_update_dof_vectors_for_continuation", &pyoomph::Problem::update_dof_vectors_for_continuation, py::arg("ddof"), py::arg("current"),
			 "Update the internally stored dof-derivative (``ddof``) and current-dof (``current``) vectors used by arclength continuation.")
		.def("_update_param_info_for_continuation", &pyoomph::Problem::update_param_info_for_continuation, py::arg("dparam_ds"), py::arg("param0"),
		...
		.def("_set_arc_length_theta_sqr", &pyoomph::Problem::set_arc_length_theta_sqr, py::arg("theta_sqr"),
			 "Set theta^2, the weighting parameter that controls the proportion of the arclength taken up by the continuation parameter.")
		.def("_set_arclength_parameter", &pyoomph::Problem::set_arclength_parameter, py::arg("name"), py::arg("value"),
		...
		.def("_start_bifurcation_tracking", &pyoomph::Problem::start_bifurcation_tracking,
			 py::arg("parameter_name"), py::arg("bifurcation_type"), py::arg("block_solve"), py::arg("eigenvector1"), py::arg("eigenvector2"), py::arg("omega"), py::arg("special_residual_forms"),
		...
		.def("_start_orbit_tracking", &pyoomph::Problem::start_orbit_tracking,
			 py::arg("history"), py::arg("period"), py::arg("bspline_order"), py::arg("gl_order"), py::arg("knots"), py::arg("T_constraint_mode"),
		...
		.def("_add_augmented_dofs", &pyoomph::Problem::add_augmented_dofs, py::arg("augmentations"),
		...
		.def("_set_lambda_tracking_real", [](pyoomph::Problem *self, double lr)
			 { *self->get_lambda_tracking_real() = lr; }, py::arg("lambda_real"), "Set the real part of lambda used for eigenbranch tracking.")
		...
		.def("_set_dof_direction_arclength", &pyoomph::Problem::set_dof_direction_arclength, py::arg("direction"),
			 "Set the direction vector along which the dof-derivative for arclength continuation is initialized/oriented.")
		.def("get_parameter_derivative", &pyoomph::Problem::get_parameter_derivative, py::arg("parameter_name"),
		...
		.def(
			...
			py::return_value_policy::reference, py::arg("parameter_name"),
		...
		.def(
			...
			py::arg("t"), "Return the values of all degrees of freedom at history/time level ``t`` (t=0: current values, t=1: previous time step, ...).")
		...
		.def(
			...
			py::arg("with_position_dofs"), "Return the current values of all pinned degrees of freedom; also includes pinned nodal position dofs if ``with_position_dofs`` is True.")
		.def(
			"set_current_dofs", [](pyoomph::Problem *self, const std::vector<double> &inp)
			{ return self->set_current_dofs(inp); },
			py::arg("values"), "Set the current values of all (unpinned and pinned) degrees of freedom from ``values``.")
		.def(
			"set_history_dofs", [](pyoomph::Problem *self, unsigned t, const std::vector<double> &inp)
			{ return self->set_history_dofs(t, inp); },
			py::arg("t"), py::arg("values"), "Set the values of all degrees of freedom at history/time level ``t`` from ``values``.")
		.def(
			...
			py::arg("values"), py::arg("with_position_dofs"), py::arg("t") = 0,
		...
		.def("_assemble_hessian_tensor", &pyoomph::Problem::assemble_hessian_tensor, py::arg("symmetric"),
		...
		.def("add_time_stepper_pt", &pyoomph::Problem::add_time_stepper_pt, py::keep_alive<1, 2>(), py::arg("time_stepper"),
			 "Add a time stepper to the problem, automatically (re-)sizing the Time object to hold the required number of history levels.")
		.def("set_mesh_pt", &pyoomph::Problem::set_mesh_pt, py::keep_alive<1, 2>(), py::arg("mesh"), "Set the (single) global mesh of the problem.")
		.def("add_sub_mesh", &pyoomph::Problem::add_sub_mesh, py::keep_alive<1, 2>(), py::arg("mesh"),
		...
		.def("get_second_order_directional_derivative", &pyoomph::Problem::get_second_order_directional_derivative, py::arg("direction"),
		...
		.def("_replace_RJM_by_param_deriv", &pyoomph::Problem::_replace_RJM_by_param_deriv, py::arg("parameter_name"), py::arg("active"),
		...
		.def(
			...
			py::arg("active"), py::arg("use_symmetry") = false,
		...
		.def("set_FD_step_used_in_get_hessian_vector_products", &pyoomph::Problem::set_FD_step_used_in_get_hessian_vector_products, py::arg("step"),
		...
		.def("mesh_pt", (oomph::Mesh * &(pyoomph::Problem::*)()) & pyoomph::Problem::mesh_pt, py::return_value_policy::reference, "Return the problem's global mesh.")
		.def("mesh_pt", (oomph::Mesh * &(pyoomph::Problem::*)(unsigned const &)) & pyoomph::Problem::mesh_pt, py::return_value_policy::reference, py::arg("i"), "Return the ``i``-th sub-mesh of the problem.")
		.def("time_pt", (oomph::Time * &(pyoomph::Problem::*)()) & pyoomph::Problem::time_pt, py::return_value_policy::reference, "Return the problem's Time object, holding the current time and time step history.")
		...
		.def("_set_ccompiler", &pyoomph::Problem::set_ccompiler, py::keep_alive<1, 2>(), py::arg("compiler"), "Set the C compiler used to compile generated element code.")
		...
		.def(
			...
			py::arg("what"), py::arg("contributions"), py::arg("params"), py::arg("hessian_vectors"), py::arg("hessian_vector_indices"),
		...
		.def(
			"_redistribute_local_to_global_double_vector", [](pyoomph::Problem *self, const py::array_t<double> &local_v)
			{
		...
		.def(
			"_redistribute_global_to_local_double_vector", [](pyoomph::Problem *self, const py::array_t<double> &global_v)
			{
		...
		.def(
			...
			py::arg("my_element"), py::arg("code_trunk"), py::arg("suppress_writing"), py::arg("suppress_compilation"), py::arg("bulkmesh"), py::arg("quiet"), py::arg("extra_flags"),
