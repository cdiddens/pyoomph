...
public:
  ...
  ~QuadTree() 
  {
    if ( split_ )
    {
      ...
      {
        //FOR PYOOMPH: this was an ASSERT, which now throws (see Helpers.h) - and throwing from a
        // destructor would call std::terminate. Skipping the corrupted child leaks it at worst,
        // which beats killing the process while it is already tearing the quadtree down.
        if ( !child )
          continue;
        delete  child;
      }
