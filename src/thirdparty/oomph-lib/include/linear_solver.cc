...
namespace oomph
{
  ...
  void SuperLUSolver::solve(Problem* const& problem_pt, DoubleVector& result)
  {
        ...
        (Solver_type == Default && problem_pt->communicator_pt()->nproc() > 1))
    {
      ...
      {
        LinearAlgebraDistribution dist(problem_pt->communicator_pt(), n_dof, problem_pt->block_dof_pt_start(), !Dist_use_global_solver); 
        this->build_distribution(dist);
      }
      ...
      else
      {
        ...
        LinearAlgebraDistribution dist(
          problem_pt->communicator_pt(), problem_pt->ndof(), problem_pt->block_dof_pt_start(), false);
  ...
  void SuperLUSolver::resolve(const DoubleVector& rhs, DoubleVector& result)
  {
    ...
    if (Doc_time)
    {
      oomph_info << "Time for LinearSolver re-solve (ndof=" << rhs.nrow() << "): " //FOR PYOOMPH: Replaced SuperLUSolver by LinearSolver, since it is not always superlu doing the job
