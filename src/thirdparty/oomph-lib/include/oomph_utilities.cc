...
namespace oomph
{
                         ...
                         const bool& make_duplicate_of_mpi_comm_world)
  {
    ...
    //MPI_Init(&argc, &argv);
    int res;
	 MPI_Initialized(&res); //FOR PYOOMPH: Patch to do not call MPI_Init twice when init from python
	 if (!res)
	 {
		     MPI_Init(&argc,&argv);
	 }
    ...
    //FOR PYOOMPH: Do not disturb here
    /*
  ...
  namespace TimingHelpers
  {
    ...
    std::string convert_secs_to_formatted_string(const double& time_in_sec)
    {
      ...
      else
      {
        ...
        ss << std::setprecision(5) << std::fixed << seconds_double << "s"; //FOR PYOOMPH: More digits
      }
