...
.. warning::
   ...
   For the same reason, you cannot set :py:class:`~pyoomph.equations.generic.DirichletBC` terms for discontinuous fields at interfaces. While usually you do not set :py:class:`~pyoomph.equations.generic.DirichletBC`\ s for the pressure, this can be problematic for the case discussed in :numref:`secspatialstokespuredirichlet`, where one degree of the pressure had to be eliminated to remove the null space. To that end, the predefined :py:class:`~pyoomph.equations.navier_stokes.StokesEquations` in :py:mod:`pyoomph.equations.navier_stokes` has a function to fix one pressure degree for both cases, Taylor-Hood and Crouzeix-Raviart elements. See :download:`stokes_pressure_fix.py` for an example how to use it.
