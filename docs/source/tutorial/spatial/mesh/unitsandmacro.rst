...
.. warning::
   ...
   On a moving mesh, i.e. whenever the nodal positions are unknowns (:py:class:`~pyoomph.equations.ALE.LaplaceSmoothedMesh`, :py:class:`~pyoomph.equations.ALE.PseudoElasticMesh`), curved entities shape the *initial* mesh only. Afterwards the boundary is free to move and pyoomph does not pull it back onto the curve -- which is what you want for a free surface, and something to keep in mind for a curved wall that is meant to stay put.
