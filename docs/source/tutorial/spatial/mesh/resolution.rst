...
.. literalinclude:: mesh_gmsh_fish_with_holes.py
   :language: python
   :start-at: self.plane_surface("mouth","fin","curved",name=self.domain_name,holes=[["eye"]])
   :end-at: self.plane_surface("mouth","fin","curved",name=self.domain_name,holes=[["eye"]])

``holes`` must take a list of holes, which again are each lists of boundary lines or boundary line names.
