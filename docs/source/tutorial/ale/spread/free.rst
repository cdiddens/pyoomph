...
Another feature which we highlight here, is the usage of the classes :py:meth:`~pyoomph.equations.ALE.ConstrainPositionsToC1Space` and :py:meth:`~pyoomph.equations.ALE.UnconstrainPositionsFromC1Space`. The former enforces that the moving mesh is solved on first order elements everywhere inside the droplet, i.e. while still having 9 nodes per second-order quadrilateral elements, the mesh dynamics is only solved on the 4 vertex nodes (``"C1"`` space). However, at the interface, we want to include the possibility of having a curved boundary. Therefore, it is unconstrained there, i.e. the free surface can still deform as second-order elements. This approach reduces the degrees of freedom from 13298 down to 8722.
