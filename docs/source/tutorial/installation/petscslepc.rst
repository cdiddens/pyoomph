...
We unfortunately do not really know how to install SLEPc natively on Windows, so you have to find your own way of installing it (and let us know the steps. A good start can be found `here <https://petsc.org/release/install/windows/>`__). If you are on Windows, the easiest way to get PETSc/SLEPc working is to install pyoomph inside the Windows Subsystem for Linux (WSL) and follow the normal Linux instructions below -- see :numref:`installwsl` for step-by-step instructions.
