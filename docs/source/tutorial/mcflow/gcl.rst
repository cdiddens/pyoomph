...
Each history term of a time derivative of an integral is evaluated on the configuration the mesh had at that step, not only in its nodal values: the normal, the Eulerian element sizes and the Eulerian spatial derivatives in ``grad``/``div`` are taken from that step's geometry as well. This is what the ``apply_on_others`` argument of :py:func:`~pyoomph.expressions.generic.evaluate_in_past` does, and :py:func:`~pyoomph.expressions.time_derivative_of_integral`, :py:meth:`~pyoomph.generic.codegen.BaseEquations.add_dweak_dt`, :py:func:`~pyoomph.expressions.generic.time_scheme` and :py:func:`~pyoomph.expressions.generic.evaluate_at_midpoint` request it by default. Spatial derivatives and normals inside such an integral are therefore fine; earlier versions evaluated them on the present mesh instead, which is why they had to be avoided there. On a static mesh nothing changes, and no history geometry is computed at all.


The conservative form can be combined with stability analysis, i.e. with solving eigenproblems. A time derivative of an integral is not a :py:func:`~pyoomph.expressions.generic.partial_t` of a field, so its mass matrix cannot be read off a single term: it is the derivative :math:`\partial I/\partial U` of the whole integral :math:`I` with respect to *every* unknown :math:`U` it depends on. Besides the obvious ones, this includes the mesh positions, through the changing integration domain, and -- whenever the mass density is composition- or temperature-dependent -- the degrees of freedom the density is built from, since :math:`\partial_t(\rho\vec{u})=\rho\partial_t\vec{u}+\vec{u}\partial_t\rho`. pyoomph assembles all of these. 

.. note::

	If the mass density depends *explicitly* on the position, write that dependence with ``var("mesh_x")`` rather than with ``var("coordinate_x")``. Both name the same position field, but they differ in their time derivative (see :numref:`seckeywordvars`): that of ``"coordinate_x"`` is zero by definition, whereas that of ``"mesh_x"`` is the mesh velocity. The conservative form evaluates :math:`\rho` at the current position inside the integral and therefore always reads such a density as attached to the laboratory frame; in the non-conservative form only ``"mesh_x"`` gives the same reading, while ``"coordinate_x"`` is taken to be attached to the moving node instead. A density built from fields, e.g. from a :py:func:`~pyoomph.materials.generic.Mixture`, is not affected by any of this -- and that is the usual case.
