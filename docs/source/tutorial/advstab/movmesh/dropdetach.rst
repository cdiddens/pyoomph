...
.. literalinclude:: hanging_droplet.py
   ...
   :end-at: self.plane_surface("axis","wall","interface",name="droplet")

The initial mesh is rather trivial, just three points, an axis of symmetry, a wall at the top and a circle segment as interface. Since the droplet will deform considerably, remeshing is required here. As discussed in :numref:`secalegmshfields`, a :py:class:`~pyoomph.meshes.gmsh.GmshTemplate` is remeshed by recreation, i.e. this very method is called again for each remeshing event. We hence check with :py:meth:`~pyoomph.meshes.mesh.MeshedMeshTemplate.is_remeshing` whether we are meshing the initial hemisphere or a deformed droplet. In the latter case, the interface is no longer a circular arc, so we reconstruct it as a spline through the current interface coordinates, obtained by :py:meth:`~pyoomph.meshes.mesh.MeshedMeshTemplate.get_boundary_coordinates` and sorted from the apex at the axis of symmetry to the contact line. The axis and the wall are unaffected by the deformation and are therefore created in the same way in both cases.
