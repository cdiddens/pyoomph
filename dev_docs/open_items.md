...
| **A moving-mesh droplet fold-track segfaults at `-n 2 --distribute`** (`hanging_droplet.py`), with `ndof` already wrong *before* tracking is switched on. Next step is stated and cheap: run it distributed with no tracking at all. | `mpi_augmented_systems.md` §6b | open, **not re-run** (a segfault under MPI takes PETSc's `MPI_ABORT` with it) |
